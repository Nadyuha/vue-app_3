  ...
  <!-- header -->
    <header class="header">
      <div class="header__wrapper container">
        <a class="header__info" @click.prevent="$router.push({name: 'main'})">
          Каталог
      </a>
        ...
        <CartIndicator/>
        <ErrorLoading v-if="productsLoadingFailed" style="width: 80%;"/>
      </div>
    </header>
...
//import PreloaderCart from './components/PreloaderCart.vue';
//import Preloader from './components/Preloader.vue';
import ErrorLoading from './components/ErrorLoading.vue';
import { mapActions, mapMutations } from 'vuex';

export default {
  components: {CartIndicator, ErrorLoading},
  data() {
        return {
  ...
  created() {
    ...
    this.loadingCart();
    if(!userAccessKey) this.productsLoadingFailed = true///!
  }, 
  methods: {
    ...mapActions(['loadCart']),
